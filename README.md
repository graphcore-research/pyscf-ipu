...
## Prepare Generated Data Bank (GDB)
QM1B was generated by taking molecules from the Generated Data Bank (GDB): https://zenodo.org/record/5172018.
...
```
To utilize caching we sorted the SMILES strings by the number of hydrogens RDKit adds to them. This means molecule `i` and `i+1` in most cases have the same number of hydrogens which allows us to reuse/cache the computational graph for DFT. This can be done by running the cells in gdb/sortgdb9.ipynb. 
