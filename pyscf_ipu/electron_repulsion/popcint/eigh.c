...
static int _dlarrk(int n, int iw, dtype gl, dtype gu, dtype *diag, dtype *e2,
                   dtype reltol, dtype *w, dtype *werr)
{
    int i, it;
    dtype mid, tmp1, left, right;
    dtype tnorm;
    int negcnt;
    int info;
    ...
    for (it = 0; it < ITMAX; it++) {
        dtype val = reltol * MAX(fabs(right), fabs(left));
        tmp1 = fabs(right - left);
        printf("it=%i, tmp=%f  upper=%f\n", it, tmp1, val);
        if (tmp1 <= 0 || tmp1 < reltol * MAX(fabs(right), fabs(left))) {
            info = 0;
            break;
        }
...
static void _dlarrc(int n, dtype vl, dtype vu,
                    dtype *diag, dtype *e2, int *lcnt, int *rcnt)
{
    int i;
    int left_count = 0;
    int right_count = 0;
    dtype tmp, lpivot, rpivot;
...
static int _dlasq4(int i0, int n0, int n0init,
                   dtype *qvecp, dtype *qvec1p, dtype *evecp, dtype *evec1p,
                   dtype *dmin, dtype *dn, dtype *tau)
{
...
static void _dlasq5(int i0, int n0,
                    dtype *qvecp, dtype *qvec1p, dtype *evecp, dtype *evec1p,
                    dtype tau, dtype tol, dtype *dmin, dtype *dn)
{
    dtype diag = qvecp[i0] - tau;
    dtype diag_min = diag;
    dtype temp;
    int j;
...
static int _dlasq2(int n, dtype *work, dtype *diag, dtype *diag_off)
{
    int i, j, itry, iwhilb, iter;
    int i0, n0, n1, n2, n0init, nbig;
    dtype emax, qmin, temp, diag_sum, tol, tol2, s, t;
    dtype dmin[3] = {0., 0., 0.};
    dtype dn[3] = {0., 0., 0.};
    dtype sigma, tau;
    dtype *qvec, *qvec1, *evec, *evec1;
    dtype *qvecp, *qvec1p, *evecp, *evec1p, *swap;
    qvec  = work;
    qvec1 = work + n * 1;
    evec  = work + n * 2;
    evec1 = work + n * 3;
...
static int _compute_eigenvalues(int n, dtype *diag, dtype *diag_off1,
                                dtype *w, dtype *werr, dtype *wgap, dtype *work)
{
    dtype gl, gu;
    dtype eabs, eold, tmp, tmp1, dmax;
    dtype eps, rtol, rtl;
    dtype sigma, tau;
    dtype sgndef, spectral_diameter;
    dtype isleft, isrght, dpivot;
    int idum, ip, i;
    int lcnt, rcnt, norep;
    int iinfo;
...
/*        store the shift for the initial RRR, which is zero in this case */
    ...
    // ok, works for: ./libcint.sh -eri -basis def2-TZVP -skipipu  -C   with dtype double (2.16e-14!)
    rtl = 1e-5;
    rtol = 1e-5; 
    eps = 1e-5; 
...
static int _dlarrf(int n, dtype *diag, dtype *diag_off1, dtype *ld, int clstrt,
                   dtype *w, dtype *wgap, dtype *werr, dtype clgapl,
                   dtype *sigma, dtype *dplus, dtype *lplus)
{
    int i, ktry;
    dtype s, tmp, max1, growthbound, lsigma;
...
static int _dlaneg(int n, dtype *diag, dtype *lld, dtype sigma, int twist_index)
{
    int j;
    int negcnt;
    dtype p, t;
...
static int _dlarrb(int n, dtype *diag, dtype *lld,
                   int ifirst, int ilast, dtype rtol1, dtype rtol2,
                   dtype *w, dtype *wgap, dtype *werr, int twist_index)
{
    int i, iter, negcnt;
    dtype mid, back, left, right, width;
    dtype cvrgd = MAX(rtol1*wgap[ifirst],
...
static void _dlar1v(int n, dtype lambda, dtype *diag, dtype *diag_off1,
                    dtype *ld, dtype *lld, dtype gaptol, dtype *vec, int *negcnt,
                    int *twist_index, dtype *resid, dtype *rqcorr, dtype *work)
{
    int i, r1, r2;
    int neg1, neg2;
    dtype s, tmp;
    dtype nrminv, mingma, dplus, dminus;
    dtype *lplus = work;
    dtype *uminus = work + n;
    dtype *work_p = work + n * 2;
...
static int _compute_eigenvectors(int n, dtype *diag, dtype *diag_off1,
                                 dtype *w, dtype *werr, dtype *wgap,
                                 dtype *vec, dtype *work, int *iwork) {
    ...
    int needbs;
    int iinfo;
    dtype fudge, eps, rqtol, tol, tmp;
    dtype left, right, gap, bstw, savgap, gaptol;
    dtype sigma, tau, resid, lambda, bstres;
    dtype rqcorr, resid_tol, rqcorr_tol;
    dtype *buf_w, *buf_ld, *buf_lld, *buf_wrk;
    int *twist_indices;
...
static int _dlaev2(dtype *eig, dtype *vec, dtype *diag, dtype *diag_off1)
{
    dtype a = diag[0];
    dtype b = diag_off1[0];
    dtype c = diag[1];
    dtype df, cs, ct, tb, sm, tn, rt, tmp;
    dtype rt1, rt2, cs1, sn1;
    int sgn1, sgn2;
...
int _CINTdiagonalize(int n, dtype *diag, dtype *diag_off1, dtype *eig, dtype *vec)
{
  // 

  // so accuracy here is ok! 
  //for (int i = 0; i < n; i++){ diag[i] = (float)diag[i]; }
  //for (int i = 0; i < n-1; i++){ diag_off1[i] = (float)diag_off1[i]; }

  for (int i = 0; i < n; i++){ printf("%f ", diag[i]);}
  printf("\n");
  for (int i = 0; i < n-1; i++){ printf("%f ", diag_off1[i]);}
  printf("\n");
  ...
  printf("eps;;; %f\n", DBL_EPSILON);
    ...
    dtype work[MXRYSROOTS * 9 + 1];
    dtype *buf_err = work + n;
    dtype *buf_gp  = work + n * 2 + 1;
    dtype *buf_wrk = work + n * 3 + 1;
    int info;
