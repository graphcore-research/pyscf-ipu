...
def cpu_our_hessenberg(A): # just change three lines of QR
    ...
    for cnt in tqdm(range(1, A.shape[0] - 1)):  # CHANGE: range(0, ..) to range(1, ..)
...
def ipu_our_hessenberg(A): # just change three lines of QR
    ...
    def body(cnt, vals):
        ...
        mask = jnp.arange(d)
        #mask = mask.at[cnt:].set(1)
        #mask = jax.lax.dynamic_update_slice(mask, jnp.ones((d-cnt)), cnt)
        #mask = jax.lax.where( jnp.arange(d) )
        mask = mask >= cnt 
        x   = R[:, cnt-1] * mask          # CHANGE: cnt to cnt-1

        #u = x + jnp.concatenate([jnp.zeros(cnt), jnp.array( jnp.copysign(jnp.linalg.norm(x), -A[cnt, cnt]) ).reshape(1), 
        #                            jnp.zeros(R.shape[0] - 1-cnt) ])

        z = jnp.zeros(x.shape)
        z = z.at[cnt].set( jnp.copysign(jnp.linalg.norm(x), -A[cnt, cnt]) )
        u = x + z
