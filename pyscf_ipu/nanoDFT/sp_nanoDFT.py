...
def nanoDFT_iteration(i, vals, opts, mol):
    ...
    E_nuc, occupancy, ERI, grid_weights, grid_AO_sp, s, grid_AO_shape, diis_history, log = vals[6:]                  # Varying types/shapes.
    ...
    E_xc, V_xc     = exchange_correlation(density_matrix, grid_AO_shape, grid_AO_sp, s, grid_weights)                # float (N, N)
    ...
    return [density_matrix, V_xc, diff_JK, O, H_core, L_inv, E_nuc, occupancy, ERI, grid_weights, grid_AO_sp, s, grid_AO_shape, diis_history, log]
...
def _sp_mm(A, B, shape):
    ...
    return jax.ops.segment_sum(prod, rows, shape).astype(jnp.float32)


def exchange_correlation(density_matrix, g_ao_shape, g_ao, s, grid_weights):
    ...
    print(f"{g_ao_shape=}")
    gsize, N = 3488, 9
    g_ao_idxs, g_ao_vals = g_ao
    filtered_idxs = jnp.take(g_ao_idxs, s, axis=1)
    g_ao_0_idxs = jnp.take(filtered_idxs, indices=jnp.arange(1, filtered_idxs.shape[0]), axis=0)
    g_ao_0_vals = jnp.take(g_ao_vals, s)
    # g_ao_dm = _sp_mm(A=(g_ao_0_idxs, g_ao_0_vals), B=density_matrix, shape=g_ao_shape[1])
    g_ao_dm = _sp_mm(A=(g_ao_0_idxs, g_ao_0_vals), B=density_matrix, shape=gsize)
    mult = g_ao_vals * g_ao_dm[(g_ao_idxs[1], g_ao_idxs[2])]

    rho = jax.lax.scatter_add(
        # operand=jnp.zeros(shape=g_ao_shape[:-1]).astype(jnp.float32), 
        operand=jnp.zeros(shape=(N, gsize)).astype(jnp.float32), 
        scatter_indices=g_ao_idxs[:-1].T,
        updates=mult,
    ...
    A_sp_rho = g_ao_vals * rho[(g_ao_idxs[0], g_ao_idxs[1])]
    sum_A_sp_rho = jax.lax.scatter_add(
        # operand=jnp.zeros(shape=g_ao_shape[1:]).astype(jnp.float32),
        operand=jnp.zeros(shape=(gsize, N)).astype(jnp.float32),
        scatter_indices=g_ao_idxs[1:].T,
        updates=A_sp_rho,
        dimension_numbers=jax.lax.ScatterDimensionNumbers(
            update_window_dims=(),
    ...
    AO_idxs_T = jnp.vstack([g_ao_0_idxs[1], g_ao_0_idxs[0]])
    # V_xc = _sp_mm(A=(AO_idxs_T, g_ao_0_vals), B=sum_A_sp_rho, shape=g_ao_shape[-1])
    V_xc = _sp_mm(A=(AO_idxs_T, g_ao_0_vals), B=sum_A_sp_rho, shape=N)
...
def _nanoDFT(state, ERI, grid_AO_sp, grid_weights, s, grid_AO_shape, opts, mol):
    ...
    E_xc, V_xc = exchange_correlation(state.density_matrix, grid_AO_shape, grid_AO_sp, s, grid_weights) # float (N, N)
    ...
    log = jax.lax.fori_loop(0, opts.its, partial(nanoDFT_iteration, opts=opts, mol=mol), [state.density_matrix, V_xc, diff_JK, state.O, H_core, state.L_inv,  # all (N, N) matrices
                                                            state.E_nuc, state.mask, ERI, grid_weights, grid_AO_sp, s, grid_AO_shape, state.diis_history, log])[-1]
...
def init_dft_tensors_cpu(mol, opts, DIIS_iters=9):
    ...
    nonzero_idxs_grid_AO = np.nonzero(grid_AO)
    nonzero_vals_grid_AO = grid_AO[nonzero_idxs_grid_AO].astype(np.float32)
    nonzero_idxs_grid_AO = np.asarray(nonzero_idxs_grid_AO, dtype=np.int32)
    grid_AO_sp           = [nonzero_idxs_grid_AO, nonzero_vals_grid_AO]
    s = np.where(nonzero_idxs_grid_AO[0]==0)[0]
    s = tuple(s.tolist())
    ...
    return state, n_electrons_half, E_nuc, N, L_inv, grid_weights, grids.coords, grid_AO, grid_AO_sp, s

def nanoDFT(mol, opts):
    # Init DFT tensors on CPU using PySCF.
    state, n_electrons_half, E_nuc, N, L_inv, _grid_weights, grid_coords, grid_AO, grid_AO_sp, s = init_dft_tensors_cpu(mol, opts)

    # grid_AO = np.transpose(grid_AO, (1, 0, 2)) # (4,gsize,N) -> (gsize,4,N)
    grid_weights = _grid_weights
    # gsize = grid_AO.shape[0]

    # remainder = gsize % opts.ndevices
    # if remainder != 0: 
    #     grid_AO = np.pad(grid_AO, ((0,remainder), (0,0), (0,0)) )
    ...
    jitted_nanoDFT = jax.pmap(partial(_nanoDFT, opts=opts, mol=mol), backend=opts.backend, 
                        in_axes=(None, eri_in_axes, [None, None], 0),
                        static_broadcasted_argnums=(4, 5),
                        axis_name="p")
    print(grid_AO.shape, grid_weights.shape)
    vals = jitted_nanoDFT(state, ERI, grid_AO_sp, grid_weights, s, grid_AO.shape)
