...
def sparse_symmetric_einsum(nonzero_distinct_ERI, nonzero_indices, dm, backend):
    ...
    def iteration(symmetry, vals): 
        ...
        def sequentialized_iter(i, vals):
            ...
            print(indices.min(), indices.max(), dm_indices.min(), dm_indices.max(), dm.shape)
...
if __name__ == "__main__":
    ...
    print("---")
    for i in range(batches):
        print(kl[:, i].min(), kl[:, i].max(), dm.shape)
    ...
    #diff_JK = sparse_symmetric_einsum(nonzero_distinct_ERI[0], nonzero_indices[0], dm, args.backend) 
