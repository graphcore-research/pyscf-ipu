...
do
        ...
        echo "$v1 $v2"
        tmux new-session -d -s $X "taskset -c $v1-$v2 python density_functional_theory.py -gdb 9 -pyscf -split $X 16 -generate -save -backend cpu  -level 0 -plevel 0 -its 50 |& tee tmp/f64$X.txt "
