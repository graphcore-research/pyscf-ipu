...
smiles = open("../gdb/GDB17.50000000", "r").read().split("\n")
...
n = len(smiles)
for i in tqdm(range(0, n, 100000)):
  times = []
  ...
  print(smile)
  atoms = [a for a in list(smile.upper()) if a == "C" or a == "N" or a == "O" or a == "S" or a == "F" or a == "Br"] #+ ["H"]
  if len(atoms) < 17: continue 
  print(atoms)
  times.append(time.time())
  ...
  locs = np.random.uniform(0,1,(17, 3)) .tolist()
  times.append(time.time())
...
plt.legend()
plt.ylabel("GDB-17 (180 B)")
#ax.set_title("GDB-11 (22 Million)")
plt.xlabel("N: input size")
plt.tight_layout()
plt.savefig("gdb17_nao_nrs.jpg")
