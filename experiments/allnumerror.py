...
for outer_num, i in enumerate([0,  2,  4, 8, 16, 24]): # plot energy aswell 
#for outer_num, i in enumerate([0, 1, 2, 24]):

  files32 = sorted([a for a in os.listdir("numerror/%s/"%sk) if int(a.split("_")[0]) == i and ".jpg" not in a and "True" in a and ".py" not in a and "allvals" not in a and "cs" not in a and "energy" not in a and "hyb" not in a and "n_electrons" not in a]  )
  #files64 = sorted([a for a in os.listdir("numerror/%s/"%sk) if int(a.split("_")[0]) == i and ".jpg" not in a and "False" in a and ".py" not in a and "allvals" not in a and "cs" not in a and "energy" not in a and "hyb" not in a and "n_electrons" not in a]  )
  files64 = files32 # sorted([a for a in os.listdir("numerror/%s/"%sk) if int(a.split("_")[0]) == i and ".jpg" not in a and "False" in a and ".py" not in a and "allvals" not in a and "cs" not in a and "energy" not in a and "hyb" not in a and "n_electrons" not in a]  )
  ...
  for num, (file32, file64) in enumerate(zip(files32, files64)):
    val32 = np.load("numerror/%s/"%sk+file32)["v"]
    val64 = np.load("numerror/%s/"%sk+file64)["v"]
    shape = val32.shape
...
plt.tight_layout()
plt.savefig("iteration_%s.jpg"%sk)