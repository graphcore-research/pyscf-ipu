...
def calculate_hs2(smile):
    ...
    for isomer in isomers:  # only store isomers we can embed. 
        val = AllChem.EmbedMolecule(isomer)
        if val == -1: continue 
        validated.append(Chem.MolToSmiles(isomer, isomericSmiles=True))  #isomericSmiles adds [ ] brackets which contain information on stereochemistry! '''
...
for i in tqdm(range(0, len(smiles), chunk)): # 100*250 = 25000k/s ; i think this is similar to not using the threads? 
  ...
  _isomers, _hs = calculate_hs2( current[0]) 
  for j, iso in enumerate(_isomers): 
    num_hs.append(_hs) 
    isomers.append(iso) 
    isocount.append(i) 
  ...
  if i % 10000 == 0: 
      df = pd.DataFrame({"smiles": isomers, "hs": num_hs, "isocount": isocount})
      df.to_parquet("%02i_v2.parquet"%number, compression="snappy")
