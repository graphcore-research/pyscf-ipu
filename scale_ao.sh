...
do
	...
	#tmux new-session -d -s t$X "taskset -c $v1-$v2 python density_functional_theory.py -gdb 9 -sk 8 -scale_ao 1e$X -backend cpu -float32  -level 0 -plevel 0 -debug  |& tee tmp/test$X.txt "
	tmux new-session -d -s t$X "taskset -c $v1-$v2 python density_functional_theory.py -gdb 9 -sk -1 -scale_ao 1e$X -backend cpu -float32  -level 0 -plevel 0 -debug  |& tee tmp/test$X.txt "
done
