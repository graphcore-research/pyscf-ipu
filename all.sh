...
do
        ...
        echo "$v1 $v2"
        tmux new-session -d -s $X "taskset -c $v1-$v2 python density_functional_theory.py -gdb 9 -split $X 16 -threads 2 -generate -save -backend cpu -float32 -level 0 -plevel 0 -its 50 |& tee tmp/test$X.txt "
