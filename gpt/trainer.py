...
class Trainer:
    ...
    def init_params(self):
        ...
        xs, ys, floats_x, floats_y = map(jnp.array, batch)

        params = self.hk_loss_fn.init(subkey, xs, ys, floats_x, floats_y)
        #logger.info("number of parameters: %d", sum([leave.size for leave in jax.tree_leaves(params)]))
        logger.info("number of parameters: %d", sum([leaf.size for leaf in jax.tree_util.tree_leaves(params)]))
        return params
    ...
    def train(self, params, opt_state=None):
        ...
        def run_epoch(params, opt_state, it, split):
            ...
            for batch in pbar:
                ...
                # forward the model
                if is_train:
                    #loss, params, opt_state = jax.jit(update, backend="ipu", donate_argnums=(0, 4))(params, subkey, xs, ys, opt_state)
                    loss, params, opt_state = jax.jit(update, backend="cpu")(params, subkey, xs, ys, opt_state)
