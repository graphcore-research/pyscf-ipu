...
do
        ...
        echo "$v1 $v2" # is more iterations enough? 
        tmux new-session -d -s $X "taskset -c $v1-$v2 python density_functional_theory.py -gdb 11 -float32 -split $X 16 -generate -save -backend cpu  -level 0 -plevel 0 -its 100 -debug |& tee tmp/f32$X.txt "
